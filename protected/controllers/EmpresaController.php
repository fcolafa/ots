...
class EmpresaController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Empresa']))
		{
			...
			$filename=$model->ID_EMPRESA.".jpg";
			$model->URL_LOGO = $filename;
			if($model->save()){
				Auditoria::model()->registrarAccion('', $model->ID_EMPRESA , $model->NOMBRE_EMPRESA);
				if(!empty($model->image)){
					$model->image->saveAs(Yii::app()->basePath.'/../archivos/empresas/'.$filename);
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Empresa']))
		{
			...
			$model->image=CUploadedFile::getInstance($model,'image');
			$filename=$id. ".jpg";
			//$model->ID_EMPRESA=$_POST['Empresa']['ID_EMPRESA'];
			if($model->save()){
				Auditoria::model()->registrarAccion('', $model->ID_EMPRESA , $model->NOMBRE_EMPRESA);
				if(!empty($model->image)){
					try{
					...
					$model->image->saveAs(Yii::app()->basePath.'/../archivos/empresas/'.$filename);
    				 ...
    				 echo "No se ha logrado actualizar el equipo ";
    				}
				}
				$this->redirect(array('view','id'=>$model->ID_EMPRESA));
			}
	...
	public function actionDelete($id)
	{
		...
		$filename=Yii::app()->basePath.'/../archivos/empresas/'.$id.".jpg";
		unlink($filename);
		}catch(Exception $e){
