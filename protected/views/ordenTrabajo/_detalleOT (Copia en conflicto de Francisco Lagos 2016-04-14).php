...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
		<table width='100%' class='table table-condensed table-hover'>
			<thead class='borde-azul'>
			  ...
			  <tr>				  	<!-- style="display:none;" -->
					...
					<td><?php
			                ...
			                CHtml::listData(CentroDeCostos::model()->findAll(),'ID_CENTRO_COSTO','NUMERO_CENTRO'),
			                        array(
			                            'ajax'=>array(
			                              'type'=>'POST',
			                              'url'=>CController::createUrl('OrdenTrabajo/SelectSubCentros'),
			                              'update'=>'#'.CHtml::activeId($new_sub_item,'ID_SUB_CENTRO_COSTO'),
			                              'beforeSend' => 'function(){
			                               $("#InsumosOT_ID_SUB_CENTRO_COSTO").find("option").remove();
			                               $("#InsumosOT_ID_CC_SECCION").find("option").remove();
			                               }',  
			                            ),'prompt'=>'Seleccione'
			                        )
			                        );
					...
					<td><?php
							/*echo $form->dropDownList($new_sub_item,'ID_SUB_CENTRO_COSTO', 
                                                             $this->getCC(), array( 'class'=>'span12')); 
							echo $form->error($new_sub_item,'ID_CENTRO_COSTO');*/
			                $lista_dos = array();
			                /*if(isset($new_sub_item->ID_SUB_CENTRO_COSTO)){
			                $id_cc = intval($new_sub_item->ID_CENTRO_COSTO); 
			                $lista_dos = CHtml::listData(SubCentroCosto::model()->findAll("ID_CENTRO_COSTO = '$id_cc'"),'ID_SUB_CENTRO_COSTO','SUC_CC_NUMERO');
			                }*/
			                echo $form->dropDownList($new_sub_item,'ID_SUB_CENTRO_COSTO',$lista_dos,
			                        array(
			                            'ajax'=>array(
			                              	'type'=>'POST',
			                              	'url'=>CController::createUrl('OrdenTrabajo/SelectSecciones'),
			                              	'update'=>'#'.CHtml::activeId($new_sub_item,'ID_CC_SECCION'),
			                              	'beforeSend' => 'function(){
			                              	$("#InsumosOT_ID_CC_SECCION").find("option").remove();
			                               	}',      
			                            ),
							...
							echo $form->error($new_sub_item,'ID_SUB_CENTRO_COSTO');
						?>
					</td>
					<td><?php
							echo $form->dropDownList($new_sub_item,'ID_CC_SECCION', 
                            $this->getCC(), array( 'class'=>'span12')); 
							echo $form->error($new_sub_item,'ID_CENTRO_COSTO');
						?>
					</td>
			...
			<thead><tr></tr><td class="text-right" colspan="8">.</td></thead>
			<tbody> <!--  subitem creados -->
				...
				foreach ($sub_items as $sub): 	?>
						<tr>
							...
							<td class="text-right"> <?=$sub->iDCCSECCION->SUC_CC_NUMERO.' ('.$sub->iDCCSECCION->SUB_CC_DESCRIPCION.')'?> </td>
							<td class="text-right"> <?=$sub->iDSUBCENTROCOSTO->SECCION_CC_NUMERO.' ('.$sub->iDSUBCENTROCOSTO->SECCION_CC_DESCRIPCION.')'?> </td>
