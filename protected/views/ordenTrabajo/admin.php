...
<!--
 <div id="msg_warnning" class="alert alert-warning">
        ...
        <ul class="notice-information">
            <li>Se ha modificado algunos iconos y añadido nuevas funciones rapidas en la grilla</li>
        </ul>
...
<?php
	...
	$data='';
        if(!Yii::app()->user->allCompany()){
            ...
            $search=$model->search();
            $empresa=  Empresa::model()->findAll('ID_EMPRESA='.Yii::app()->getSession()->get('id_empresa'));
        }
        else{
			...
			array(	'class'=>'CButtonColumn',
					...
					/*	'more' => array(
						...
						'update' => array(
                                                    ...
                                                    'visible'=> '$data->VOBO_GERENTE_GRAL!=1&&(Yii::app()->user->LOG()||Yii::app()->user->A1() || Yii::app()->user->ADM() || Yii::app()->user->GG() || Yii::app()->user->GOP() || Yii::app()->user->JDP()|| Yii::app()->user->OP())'),
