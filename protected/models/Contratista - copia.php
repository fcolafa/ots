 ...
 * @property Ot[] $ots
 * @property SubItemPresupuesto[] $subItemPresupuestos
 */
class Contratista extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('ID_CONTRATISTA, ID_EMPRESA, AUTORIZADO', 'numerical', 'integerOnly'=>true),
			array('NOMBRE_CONTRATISTA, CIUDAD_CONTRATISTA, ENCARGADO', 'length', 'max'=>150),
			array('RUT_CONTRATISTA', 'validateRut'),
			array('RUT_CONTRATISTA', 'compruebaExistencia'),
			array('RUT_CONTRATISTA', 'length', 'max'=>15),
			array('TELEFONO_CONTRATISTA', 'length', 'max'=>250),
			array('GIRO_AREA', 'length', 'max'=>50),
			array('AUTORIZADO', 'length', 'max'=>1),
			array('DIRECCION_CONTRATISTA', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('ID_CONTRATISTA, GIRO_AREA, NOMBRE_CONTRATISTA, AUTORIZADO, RUT_CONTRATISTA, DIRECCION_CONTRATISTA, CIUDAD_CONTRATISTA, TELEFONO_CONTRATISTA, ENCARGADO', 'safe', 'on'=>'search'),
		);
	}
	...
	public static function model($className=__CLASS__)
	{
		...
		public function validateRut($attribute, $params) {
			$rut =str_replace ("." , "", $this->RUT_CONTRATISTA);
	        $data = explode('-', $rut);
	        if(ctype_digit($data[0])){
		        ...
		        if ($verifyCode != $result)
		            $this->addError('RUT_CONTRATISTA', 'Rut no válido, verifique el dato ingresado"');
			    }else{
			    	$this->addError('RUT_CONTRATISTA', 'Rut inválido. Datos no coinciden con el formato de rut"');
			    }
		}
	  ...
	  public function compruebaExistencia($attribute,$params){
	    if(!strpos($_SERVER['REQUEST_URI'], 'update'))
			{
				$dato = $this->RUT_CONTRATISTA;
				$temp= Contratista::model()->findByAttributes(array('RUT_CONTRATISTA'=>$this->RUT_CONTRATISTA));

				if($temp!=null)
					$this->addError('RUT_CONTRATISTA','Rut de empresa contratista existente en el sistema');
			}
	  }
